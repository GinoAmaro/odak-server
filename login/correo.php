...
function enviarSeguimiento($correo, $usuario, $seguimiento, $descripcion)
{
    ...
    try {
        ...
        </p>' . $descripcion . '<br/><br/>Tu N° de seguimiento es:<b> ' . $seguimiento . '</b> <p style="font-family: Poppins"> <strong>Puedes revisar la solicitud en el siguiente <a href="http://localhost:4200/odak/seguimiento">LINK</a></strong><br/><br/> Team Touch</p> </div>';
        $mail->AltBody = 'This is the body in plain text for non-HTML mail clients';
...
function enviarRepuesta($correo, $usuario, $seguimiento, $descripcion)
{
    ...
    if ($descripcion == 'Aceptada') {
        ...
        $texto = 'Tu solicitud ha sido aceptada.';
        $texto_dos = 'Recuerda revisar tu número de seguimiento en el siguiente <a href="http://localhost:4200/odak/seguimiento">LINK</a> ';
    ...
    if ($descripcion == 'Rechazada') {
        ...
        $texto_dos = 'Siempre puedes volver a buscar lo que necesitas en el siguiente <a href="http://localhost:4200/odak/pyme">LINK</a> ';
        $texto_tres = '';
        $texto_cuatro = 'Nos vemos pronto!.';
        $seguimiento = '';
    }
...
function enviarTarea($correo, $usuario, $empresa, $descripcion)
{
    ...
    try {
        ...
        $mail->Host       = 'mail.labtouch.cl';                     //Set the SMTP server to send through
        $mail->SMTPAuth   = true;                                   //Enable SMTP authentication
        $mail->Username   = 'alerta@labtouch.cl';                     //SMTP username
        $mail->Password   = '';                               //SMTP password
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;            //Enable implicit TLS encryption
        $mail->Port       = 465;                                    //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`

        //Recipients
        $mail->setFrom('alerta@labtouch.cl', 'ODAK');
        $mail->addAddress($correo, $usuario);     //Add a recipient
        // $mail->addAddress('ellen@example.com');               //Name is optional
        // $mail->addReplyTo('info@example.com', 'Information');
        // $mail->addCC('cc@example.com');
        ...
        //Content
        $mail->isHTML(true);                                  //Set email format to HTML
        $mail->Subject = 'Nueva Tarea, de empresa: ' . $empresa;
        $mail->Body    = '<div align="center">
                          <img src="https://labtouch.cl/img/marca.png" width="308">
                          <p style="font-family: Poppins">
                          Hola ' . $usuario . ' (Este es un email automático, no responder por favor)
                          <p style="font-family: Poppins"> Tienes una nueva tarea de empresa: <b>' . $empresa . '</b><br> 
                          </p>La tarea asignada es: <br/><br/>' . $descripcion . '<br/><strong>Saludos!</strong> Team Touch</p> </div>';
        $mail->AltBody = 'This is the body in plain text for non-HTML mail clients';
